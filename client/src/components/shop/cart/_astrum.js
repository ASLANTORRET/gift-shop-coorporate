...
import { addToCartAstrum, removeFromCartAstrum } from '../../../actions/cart'
import HtmlContent from '../../html-content'

const Astrum = ({ item, addToCart, removeFromCart }) => {
  const { product } = item
  return (
    <tr>
      <td className="cart-image">
        <Link to={`/astrum/product/${ product.id }`}>
          <img src={ product.pictures[0] } alt={ product.name_rus }/>
        </Link>
      </td>
      <td className="cart-ttl">
        <Link to={`/astrum/product/${ product.id }`}>
          <HtmlContent html={product.name_rus}/>
        </Link>
      </td>
...
const withState = connect(
  ...
  (dispatch) => ({
    ...
    removeFromCart(id) {
      dispatch(removeFromCartAstrum({id}))
    },
  })
)


export default withState(Astrum)